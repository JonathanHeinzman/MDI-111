...
//  TrailMarkCore
//
//  Created by Jonathan Heinzman on 8/10/26.
//
...
public final class MotionManager {
    ...
    // Raw Gyro Data
    public private(set) var gyroscope: (Double, Double, Double) = (0, 0, 0)
    ...
    private func startPedometer() {
        ...
        let startOfDayToday = Calendar.current.startOfDay(for: Date()) // 2026-08-10T00:00.000-06:00
        
        pedometer.startUpdates(from: startOfDayToday) { [weak self] data, error in
            guard let data else { return }
            let steps = data.numberOfSteps.intValue
            // Cadence/Pace is always given in steps per second (steps/second) -> steps/minute
    ...
    private func startAcceqelometer() {
        guard motionManager.isDeviceMotionAvailable else { return }
        motionManager.deviceMotionUpdateInterval = 0.2 // 1/10 = 10hz, 1/5 = 5hz, 1/2 = 2hz
        motionManager.startDeviceMotionUpdates(to: .main) { [weak self] motion, error in
            self?.acceleration = (motion?.userAcceleration.x ?? 0, motion?.userAcceleration.y ?? 0, motion?.userAcceleration.z ?? 0)
            self?.gyroscope = (motion?.rotationRate.x ?? 0, motion?.rotationRate.y ?? 0, motion?.rotationRate.z ?? 0)
        }
    }
